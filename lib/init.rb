...
hal_fulton    = Library::Author.new(
                        'Hal Fulton',
                        'First programmed with Ruby in 1999')#.save_to_file
sam_ruby      = Library::Author.new(
                        'Sam Ruby',
                        'Prominent software developer')#.save_to_file
roman_snitko  = Library::Author.new(
                        'Roman Snitko', 
                        'Programmer: Ruby, Dart, Javascript, Bitcoin')#.save_to_file
alex_kruglov  = Library::Author.new(
                        'Alexander Kruglov', 
                        'Rubyist')#.save_to_file
...
book_ruby    = Library::Book.new('Agile development RAILS4',          sam_ruby)     #.save_to_file
book_snitko  = Library::Book.new('True Development on Ruby on Rails', roman_snitko) #.save_to_file
book_kruglov = Library::Book.new('Ruby',                              alex_kruglov) #.save_to_file
book_carlson = Library::Book.new('True Development on Ruby on Rails', lucas_carlson)#.save_to_file
...
# Orders
order_for_first   = Library::Order.new(book_fulton,  reader_first) #.save_to_file
order_for_first2  = Library::Order.new(book_ruby,    reader_first) #.save_to_file
order_for_second  = Library::Order.new(book_ruby,    reader_second)#.save_to_file
order_for_third   = Library::Order.new(book_snitko,  reader_third) #.save_to_file
order_for_fourth  = Library::Order.new(book_carlson, reader_fourth)#.save_to_file
order_for_fourth2 = Library::Order.new(book_ruby,    reader_fourth)#.save_to_file
order_for_fourth3 = Library::Order.new(book_fulton,  reader_fourth)#.save_to_file
...
library.add_orders(order_for_first)
library.add_orders(order_for_first2)
library.add_orders(order_for_second)
library.add_orders(order_for_third)
library.add_orders(order_for_fourth)
library.add_orders(order_for_fourth2)
library.add_orders(order_for_fourth3)
